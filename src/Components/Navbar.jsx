...
const Navbar = () => {
  ...
  return (
    <motion.nav 
      ...
      whileInView="show"
      viewport={{ once: true }}
      className="top-0 left-0 right-0 bg-transparent/100 backdrop-blur-sm z-50 shadow-sm"
    >
      <div className="w-full flex justify-between items-center container mx-auto px-4 sm:px-6 lg:px-8 md:h-20 h-16">
        ...
        >
          <motion.div 
            ...
            className="w-4 h-4 rounded-full opacity-75 font-bold hover:opacity-100 transition-opacity"
          >BeauRoi</motion.div>
          <motion.div 
            whileHover={{ scale: 1.1 }}
            className="w-4 h-4 rounded-full -ml-2 hover:opacity-75 transition-opacity"
          ></motion.div>
        </motion.div>
        {/* Mobile Menu Button */}
        <motion.button 
